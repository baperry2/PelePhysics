...
{
    amrex::GpuArray<amrex::Real,3969> Jac = {0.0};
    amrex::GpuArray<amrex::Real,62> conc = {0.0};
    for (int n=0; n<62; n++) {
        conc[n] = 1.0/ 62.000000 ;
    }
...
void SPARSITY_PREPROC_CSC(int *  rowVals, int *  colPtrs, const int * consP, int NCELLS)
{
    ...
    for (int nc=0; nc<NCELLS; nc++) {
        int offset_row = nc * 63;
        int offset_col = nc * 63;
        for (int k=0; k<63; k++) {
            for (int l=0; l<63; l++) {
                if(Jac[63*k + l] != 0.0) {
